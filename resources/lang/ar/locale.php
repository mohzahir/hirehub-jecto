<?php

return [
    "Home" => "الرئيسية",
    "About" => "عن الشركة",
    "Services" => "الخدمات",
    "Contact" => "تواصل معنا",
    "Register" => "تسجيل جديد",
    "Login" => "تسجيل دخول",
    "Jobs" => "الوظائف",
    "We have" => "لدينا أكثر من",
    "live jobs" => "وظيفة متاحة الان",
    "Next Future Job Waiting For You Hold It!" => "وظيفتك المستقبلية في انتظارك لتمتلكها!",
    "Search Your Job" => "ابحث عن وظيفتك",
    "All Categories" => "جميع الأقسام",
    "location" => "الموقع",
    "Search Job" => "إبحث عن الوظيفة",
    "Trending Keywords" => "كلمات مفتاحية متداولة",
    "Over" => "أكثر من",
    "supporter all over the world" => "داعم حول العالم",
    "Employers Offering Job" => "وظائف الشركات",
    "Most Featured Jobs" => " إعلانات الوظائف المميزة لدينا",
    "Explore New Life" => "إكتشف حياتك الجديدة",
    "Be Found Put Your Cv In Front Of Great Employers" => "ضع سيرتك الذاتية بين يدي افضل خبراء التوظيف",
    "CEO Message" => "رسالة المدير العام",
    "Upload Your Resume" => "إرفق سيرتك الذاتية",
    "Job By Location" => "حسب المكان",
    "Jobs By Roles" => "حسب الادوار",
    "Jobs By Skills" => "حسب المهارات",
    "Featured Job Departments We Have" => "أقسام الوظائف المميزة لدينا",
    "Working Process" => "الية العمل",
    "See How It Works" => "خطوات الحصول على وظيفة",
    "Apply For Job" => "التقديم للوظيفة",
    "apply for a job description" => "ستقوم بالتقديم على الوظيفة التي تناسب مهاراتك وخبراتك العملية عبر ملئ حقول التقديم المطلوبة وارسالها ",
    "Filtering Applications" => "تصفية التقديمات",
    "filtering applications description" => "يتم تصفية طلبات التقديم من قبل ادارة الموقع حتى الوصول الى القائمة الاكثر تناسبا للوظيفة المعلنة ",
    "Contact For Interview" => "التواصل مع الشخصيات المختارة",
    "contact for interview description" => "سيتم التواصل مع الاشخاص الذين تم اختيارهم وتحديد موعد للمعاينات الشخصية ",
    "Our Candidates" => "أميز المتقدمين لدينا",
    "Our Blogs" => "المدونة",
    "Recent News Post" => "اخر المقالات المنشورة",
    "Browse All Blogs" => "تصفح جميع المقالات",
    "Testimonials" => "اراء العملاء",
    "What Our Customers Say About Us" => "ماذا يقول عملاءنا عننا",
    "Results Found" => "عدد الوظائف",
    "Sort By" => "ترتيب حسب",
    "Find A Job" => "ابحث عن وظيفة",
    "Email address" => "البريد الالكتروني",
    "Get A Job Alert" => "الحصول على تنبيه وظيفي",
    "Search Keywords" => "كلمات البحث",
    "Keyword" => "كلمة دلالية",
    "Category" => "القسم",
    "Location List" => "قائمة البلدان",
    "Job Type" => "نوع الوظيفة",
    "Salary" => "الراتب",
    "Date Posted" => "زمن النشر",
    "Industry" => "المجال",
    "Filter Now" => "ابحث الآن",
    "Contact Now" => "تواصل الان",
    "Next" => "التالي",
    "Prev" => "السابق",
    "fulltime" => "دوام كامل",
    "parttime" => "دوام جزئي",
    "freelancer" => "مستقل",
    "remote" => "عن بعد",
    "temporary" => "وظيفة مؤقتة",
    "internship" => "تدريب وظيفي",
    "Title" => "العنوان",
    "Apply Now" => "تقدم الان",
    "Job Information" => "معلومات الوظيفة",
    "Full Name" => "الاسم الكامل",
    "Email" => "الايميل",
    "Phone" => "رقم الهاتف",
    "CV" => "السيرة الذاتية",
    "Cover Latter" => "رسالة التقدم",
    "Submit Application" => "طلب استقدام",
    "Location" => "المكان",
    "Posted Date" => "زمن النشر",
    "Experience" => "الخبرة",
    "Applied" => "عدد المتقدمين",
    "Related Jobs" => "وضائف ذات صلة",
    "Newsletter" => "النشرة البريدية",
    "Useful Links" => "روابط مهمة",
    "years" => "سنوات",
    "Duration" => "مدة التعاقد",
    "Featured" => "مميز",
    "Open Job" => "وظائف متاحة",
    "Password" => "كلمة السر",
    "Login Here" => "تسجيل الدخول",
    "Dashboard" => "لوحة التحكم",
    "Basic Info" => "المعلومات الأساسية",
    "Your Name" => "إسمك بالكامل",
    "Your Email" => "بريدك الإلكتروني",
    "Date Of Birth" => "تاريخ الميلاد",
    "Job Title" => "عنوان الوظيفة",
    "Position" => "المسمى الوظيفي",
    "Cover Picture" => "صورة خلفية",
    "Education" => "التعليم",
    "Degree" => "الدرجة العلمية",
    "Year" => "السنة",
    "Institute" => "المعهد\الجامعة",
    "Social Links" => "روابط التواصل الإجتماعي",
    "Twitter" => "تويتر",
    "Facebook" => "فيسبوك",
    "Instagram" => "انستاقرام",
    "Linkedin" => "لينكدان",
    "Save Your Information" => "حفظ البيانات",
    "Field" => "المجال",
    "My Profile" => "الملف الشخصي",
    "Applied Jobs" => "الوظائف المتقدم لها",
    "Logout" => "تسجيل خروج",
    "Save" => "حفظ",
    "Share" => "مشاركة",
    "Report" => "تبليغ",
    "Workshops" => "ورش العمل",
    "We Are A Team Of Experts In This Field" => "نحن فريق من الخبراء في هذا المجال",
    "Get Started" => "إبدأ الان",
    "TOTAL JOBS" => "عدد وظائف",
    "EMPLOYERS" => "شركات",
    "JOB APPLICATIONS" => "تقديمات وظائف",
    "Our Possibilities" => "ما يميزنا",
    "Our Company Possibilities" => "نحن نمتاز بـ",
    "Excellence" => "التفوق",
    "Adaptability" => "القدرة على التكيف",
    "Integrity" => "التكامل",
    "Innovation" => "التعاون",
    "The Passion" => "الرغبة",
    "Knowledge" => "المعرفة",
    "Browse all talent" => "تصفح جميع المواهب",
    "Candidates" => "المرشحين",
    "Name" => "الإسم",
    "Subject" => "الموضوع",
    "Message" => "الرسالة",
    "Send Message" => "إرسال",
    "Currency" => "العملة",
    "Show More" => "Show More",
    "Blog" => "المقالات",
    "Blog Details" => "تفاصيل المدونه",
    "Categories" => "الأقسام",
    "Recent Posts" => "الاحدث نشرا",
    "Comments" => "التعليقات",
    "Comment" => "تعليق",
    "Leave A Comment" => "أكتب تعليقك",
    "Post A Comment" => "انشر التعليق",
    "News Tags" => "إشارات الاخبار",
    "Tags" => "الإشارات",
    "Write message" => "اكتب رسالتك",
    "dont have account" => "ليس لديك حساب",
    "Password" => "كلمة السر",
    "Confirm Password" => "تأكيد كلمة السر",
    "Register Here" => "تسجيل",
    "CV Writing" => "كتابة السيرة الزاتية",
    "All" => "الكل",
    "STEP 01" => "الخطوة 01",
    "STEP 02" => "الخطوة 02",
    "STEP 03" => "الخطوة 03",
    "Select your CV Category" => "إختر فئة السيرة الزاتية",
    "Apply For CV Writing" => "التقديم لكتابة السيرة الذاتية",
    "Some Of CV Samples" => "بعض نمازج السير الذاتية",
    "Get your CV Now" => "احصل على سيرتك الذاتية الان",
    "CV Application Form" => "طلب كتابة سيرة ذاتية",
    "Service Information" => "معلومات الخدمة",
    "Service Type" => "نوع الخدمة",
    "CV Category" => "فئة السيرة الذاتية",
    "Dollar Price" => "السعر بالدولار",
    "SDG Price" => "السعر بالجنيه السوداني",
    "Payment Currency" => "عملة الدفع",
    "Select Payment Currency" => "إختر عملة الدفع",
    "SDG" => "جنيه سوداني",
    "USD" => "دولار امريكي",
    "Notes" => "ملاحظات",
    "CV Writing Application" => "التقديم لكتابة سيرة زاتية",
    "Start Date" => "تاريخ البداية",
    "Finish Date" => "تاريخ النهاية",
    "Workshop" => "ورشة عمل",
    "Workshop Information" => "تفاصيل ورشة العمل",
    "Total Hours" => "عدد الساعات الكلي",
    "Lecture Hours" => "ساعات المحاضرة",
    "Overview" => "نظرة عامة",
    "Trainer" => "المدرب",
    "Running Workshop Application" => "التسجيل في ورشة عمل",
    "Running Workshop Application Form" => "بيانات التسجيل",
    "Workshop Register" => "تسجيل",
    "Newest" => "الأحدث",
    "Registered Workshops" => "ورش العمل المسجلة",
    "CV Writing Requests" => "طلبات كتابة السيرة الذاتية",
    "Human Resource (HR)" => "الموارد البشرية",
    "Recruitment/Jobs" => "الوظائف",
    "Curriculum Vitae (CV) Writing" => "خدمة كتابة السيرة الذاتية",
    "Employment Clinic/Workshops" => "عيادة التوظيف\ورش العمل",
    "Our Services" => "خدماتنا",
    "Address" => "العنوان",
    "Open" => "مواعيد العمل",
    "Private Driver" => "سائق خاص",
    "Worker" => "عامل",
    "Delivery" => "توصيل",
    "Security Guard" => "حارس أمن",
    "Learn More" => "شاهد المزيد",
    "Job Categories" => "أقسام الوظائف",
    "Jobs are waiting for you" => "وظائفنا بانتظارك",
    "Browse All Categories" => "تصفح جميع الاقسام الوظيفية",
    "More of workshops are waiting for you" => "المزيد من ورش العمل بانتظارك",
    "Browse All Workshops" => "تصفح جميع ورش العمل",
    "Employment Clinic Workshops" => "ورش عمل عيادة التوظيف",
    "Testimonials" => "اراء العملاء",
    "Our Trusted Clients" => "مايقوله عملائنا عننا",
    "CV Writing Working Process" => "طريقة الحصول على سيرة زاتية",
    "Contact And Sending CV" => "التواصل وارسال السيرة الذاتية",
    "Contact And Sending CV Description" => "التواصل وارسال السيرة الذاتية",




];
